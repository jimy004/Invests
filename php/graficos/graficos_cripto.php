...
foreach ($criptos as $c) {
    $valor_total = $c['valor_actual'] * $c['cantidad'];
    $distribucion[] = ['name' => $c['nombre'], 'y' => $valor_total];
}

// --- 2️⃣ y 3️⃣ Historial ---
$sql = "SELECT fecha, valor, cantidadBTC FROM historialcriptos ORDER BY fecha ASC";
$stmt = $conexion->prepare($sql);
$stmt->execute();
