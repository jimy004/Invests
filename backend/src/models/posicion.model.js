...
export const getAllPosiciones = () => pool.query(BASE_SELECT);

export const getPosicionById = (id) =>
  pool.query(`${BASE_SELECT} WHERE p.id=?`, [id]);

export const getPosicionesByPortafolio = (portafolio_id) =>
  pool.query(
    `${BASE_SELECT} WHERE p.portafolio_id=? ORDER BY (p.cantidad * p.preciopromedio) DESC`,
    [portafolio_id]
  );

export const getPosicionesByUsuario = (usuario_id) =>
  pool.query(
    `${BASE_SELECT} WHERE po.usuario_id=? ORDER BY (p.cantidad * p.preciopromedio) DESC`,
    [usuario_id]
  );

export const getPosicionesByActivo = (activo_id) =>
  pool.query(`${BASE_SELECT} WHERE p.activo_id=?`, [activo_id]);

export const getPosicionesByActivoUsuario = (activo_id, usuario_id) =>
  pool.query(`${BASE_SELECT} WHERE p.activo_id=? AND po.usuario_id=?`, [activo_id, usuario_id]);
...
export const updatePosicionCantidad = (id, cantidad) =>
  pool.query("UPDATE Posicion SET cantidad=? WHERE id=?", [cantidad, id]);

export const updatePosicionPrecio = (id, preciopromedio) =>
  pool.query("UPDATE Posicion SET preciopromedio=? WHERE id=?", [preciopromedio, id]);

export const deletePosicion = (id) =>
  pool.query("DELETE FROM Posicion WHERE id=?", [id]);
...
export const buscarPosicionPorPortafolioActivo = (portafolio_id, activo_id) =>
  pool.query(
    "SELECT * FROM Posicion WHERE portafolio_id=? AND activo_id=?",
